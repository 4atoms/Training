...
const generate=()=>
{
        ...
        xhr.onload=(resp)=>{
            ...
            for(let i=0;myjson.flights[i]!=undefined;i++)
            {
                let namem=myjson.flights[i].name;
                let urlm=myjson.flights[i].url;
                arr3.push({namem,urlm});
            }
        }
        ...
        for (var i=0;i<arr3.length;i++){
            ...
            console.log("url",i,arr3[i].urlm);
            div.innerHTML=`Flight Name: <h2>${arr3[i].namem}</h2> <button class="button" id="tryal" onclick="tryal(${arr3[i].urlm})">VIEW</button>`;
            but.innerHTML="load";
            ele.appendChild(div);
            console.log("entered");
           }
...
const tryal=(urlmm)=>{
    console.log("entered tryal func");
    let x=10;
    //let urlm="https://run.mocky.io/v3/eb094b79-1efd-4d2b-a408-2b24d8f00055";
